      ...
      .filter(
        (s) =>
          ...
          (s.filepath !== currentFilepath &&
            // Exclude Chip's own output as it makes it super-hard for users to test the autocomplete feature
            // while looking at the prompts in Chip's output
            !s.filepath.startsWith(
              "output:extension-output-futureatoms.chip",
            )),
      )
      .sort((a, b) => b.timestamp - a.timestamp)
