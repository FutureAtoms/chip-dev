...
const ChipLogo: React.FC<ChipLogoProps> = ({ height = 40, width, className }) => {
  const aspectRatio = 3; // Approximate width:height ratio for logo with text
  const computedWidth = width ?? height * aspectRatio;
  ...
  return (
    ...
    >
      ...
      {/* "Chip" text */}
      <text x="45" y="27" fontSize="18" fontWeight="bold" fill="currentColor" fontFamily="system-ui, -apple-system, sans-serif">
        Chip
      </text>
    </svg>
  );
